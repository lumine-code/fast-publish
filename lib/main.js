...
module.exports = {
  ...
  async gitPrepare(cwd, version) {
    ...
    try {
      ...
      atom.notifications.addSuccess(`Fast Publish: ${pkgName} ${tag} published`);
    } catch (err) {
      atom.notifications.addError(`Fast Publish: ${pkgName} failed`, {
  ...
  async publish(dirPath, mode) {
    ...
    try {
      ...
      if (reason) {
        atom.notifications.addWarning(`Fast Publish: ${pkgName} not released`, {
